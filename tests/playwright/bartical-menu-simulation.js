  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      heroVideo: heroVideo ? {
        ...
        loopFadeMs: heroVideo.dataset.loopFadeMs,
...
function assertInitialState(state, viewport) {
  ...
  assert(
    state.pageBackgroundColor === 'rgb(24, 27, 58)'
      ...
      && state.heroVideo.loopFadeMs === '1400'
...
async function verifyInteractions(page, viewport) {
  ...
  const distributionLayout = await distributionAction.evaluate((action) => {
    ...
    return {
      ...
      iconColor: iconStyle?.backgroundColor || null,
      transitionDuration: actionStyle.transitionDuration,
      iconMask: iconStyle ? `${iconStyle.maskImage} ${iconStyle.webkitMaskImage}` : '',
      iconIsLeftOfLabel: Boolean(labelRect && iconRect && iconRect.right <= labelRect.left)
    };
  });
  assert(
    distributionLayout.href === 'https://masashi-desu.itch.io/bartical'
      && distributionLayout.target === '_blank'
      && distributionLayout.rel?.includes('noopener')
      && distributionLayout.rel.includes('noreferrer')
      && distributionLayout.label === 'itch.ioで入手'
      && distributionLayout.backgroundColor === 'rgb(255, 255, 255)'
      && distributionLayout.labelColor === 'rgb(23, 21, 124)'
      && distributionLayout.iconColor === distributionLayout.labelColor
      && distributionLayout.transitionDuration === '0s'
      && distributionLayout.iconMask.includes('cdn.jsdelivr.net/npm/simple-icons@v16/icons/itchdotio.svg')
      && distributionLayout.iconIsLeftOfLabel,
    `[${viewport.name}] Bartical distribution action did not keep its label and left-side Simple Icons mark legible on the published itch.io link`,
    distributionLayout
  );
