...
- `npm run test:segmented-scroll-core`: mock clock で小数入力、しきい値、長い入力・慣性・反転、端、移動中の入力、静止後の再操作、単位変換、ズーム、destroy/remount を検証する。
- `npm run test:segmented-wheel`: Chromium の実ページでしきい値とロック、active nav と位置の一致、横ナビ、実 wheel 入力を検証する。`test:pc-browser` に含める。
