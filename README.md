...
1. `npm test` を実行し、全自動テストをパスさせる。
2. `npm test` 内の `npm run test:pc-browser` によるChromiumのPCブラウザテストがパスしていることを確認し、本番ビルドの対象画面を通常のPCブラウザでも目視確認する。
3. `npm test` 内の `npm run test:webkit` によるPlaywright WebKitテストがすべてパスしていることを確認する。
4. `npm run build` と `npm run preview` で本番ビルドを配信し、`xcrun simctl boot <UDID>` で起動した利用可能な iPhone Simulator の Mobile Safari で対象画面を確認する。`xcrun simctl bootstatus <UDID> -b` で起動完了を待ってから確認する。
5. PCブラウザとiPhone Simulatorの確認では、変更箇所のレイアウト、操作、主要な視覚効果、横方向のオーバーフロー、コンソールエラーの有無を確認する。
6. `git status --short` でリリース対象を確定し、一時的なスクリーンショットやログが `.temp/` の外やステージ対象に混入していないことを確認する。
...
GitHub Pages を公開するすべての workflow も `npm test` を品質ゲートとして実行します。通常のPages workflowは失敗時にビルド成果物を公開せず、TypeFetch appcastの自動同期workflowは失敗時に `main` の更新も公開も行いません。CIはPCブラウザとWebKitの自動テストを再検証しますが、`xcrun simctl boot` によるiPhone Safari確認と目視確認の代替にはならないため、これらはローカルのpush前ゲートとして必須です。
...
itch.io 公開後は同じ workflow が `operation=sync` で呼ばれ、同じ入力検証を通過した場合だけ appcast を生成します。`dc:identifier` に `TypeFetch@<source_sha>` を記録し、build の巻き戻しと、同一 build の別 version／source への再割り当ては拒否します。appcast の生成・単体検証に加えて `npm test` の全品質ゲートが成功した場合だけ正本を `main` へコミットし、書き込み権限を持たない別 job がその commit を Vite build して、同じ workflow run で Pages artifact をデプロイします。
