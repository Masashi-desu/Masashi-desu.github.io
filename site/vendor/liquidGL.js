 ...
 * Licence: MIT
 * Version: v2.0.1
 * Upstream commit: 28e7c1a64a96c46449e9f494acee98c8490d9824
 * Local integration: snapshot recovery, dynamic-video rescan, and object-fit sync.
 */
  ...
  class liquidGLRenderer {
    constructor(
      ...
      this.canvas.addEventListener("webglcontextlost", (event) => {
        ...
        emitRendererEvent("liquidgl:context-lost", { renderer: this });
      });
      this.canvas.addEventListener("webglcontextrestored", () => {
        ...
        this.textureWidth = 0;
        this.textureHeight = 0;
        this._vBlitReady = undefined;
        this._vFboTexture = null;
        try {
          this._initGL();
          this._resizeCanvas();
          this.lenses.forEach((lens) => lens.updateMetrics());
          emitRendererEvent("liquidgl:context-restored", { renderer: this });
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          const snapCanvas = await timeoutPromise(
            NaughtyDOM.rasteriseAsync(this.snapshotTarget, {
              width: fullW,
              height: fullH,
              scale: scale,
    ...
    _renderLens(lens) {
      ...
      const scaleY = canvasRect.height > 0
        ...
        : Math.min(2, window.devicePixelRatio || 1);
      const leftPx = (rect.left - canvasRect.left) * scaleX;
      const topPx = (rect.top - canvasRect.top) * scaleY;
      const x = Math.round(leftPx);
      const yTop = Math.round(topPx);
      const w = Math.round(leftPx + rect.width * scaleX) - x;
      const h = Math.round(topPx + rect.height * scaleY) - yTop;
      const y = this.canvas.height - (yTop + h);

      gl.viewport(x, y, w, h);
      gl.uniform2f(this.u.res, w, h);
      gl.uniform2f(this.u.subpixel, leftPx - x, topPx - yTop);
      gl.uniform2f(this.u.boxSize, rect.width * scaleX, rect.height * scaleY);
    ...
    refreshDynamicMedia() {
      ...
      const changed =
        nextVideos.length !== this._videoNodes.length ||
        nextVideos.some((video, index) => video !== this._videoNodes[index]);
      this._videoNodes = nextVideos;
      if (changed) {
        this._videoFrameState = new WeakMap();
        this._videoAlphaState = new WeakMap();
      }
      return this._videoNodes.length;
    }

    _getVideoSourceRect(video, style, targetWidth, targetHeight) {
      ...
      const x = (sourceWidth - width) * positionX;
      const y = (sourceHeight - height) * positionY;
      return {
        x,
        y,
        width,
        ...
        u: x / sourceWidth,
        v: y / sourceHeight,
        uw: width / sourceWidth,
        vh: height / sourceHeight,
      };
    }
    ...
    _updateDynamicVideos() {
      ...
      this._videoNodes.forEach((vid) => {
        ...
        const sourceRect = this._getVideoSourceRect(vid, style, drawW, drawH);
        const scaledRadii = {
        ...
        if (
          ...
          this._blitVideoToTexture(vid, dstX, dstY, updW, updH, {
            u: sourceRect.u + sourceRect.uw * (srcX / drawW),
            v: sourceRect.v + sourceRect.vh * (srcY / drawH),
            uw: sourceRect.uw * (updW / drawW),
            vh: sourceRect.vh * (updH / drawH),
          })
        ) {
          return;
        }
    ...
    _reveal() {
      ...
      const dur =
        typeof this.options.revealDuration === "number"
          ? Math.max(0, this.options.revealDuration)
          : 1000;
      const start = performance.now();

      const animate = () => {
        if (revealGeneration !== this.renderer._revealGeneration) return;
